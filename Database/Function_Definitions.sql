...
/*UTILIZE THIS FUNCTION WITH THE FOLLOWING STATEMENT:
    CALL LOG_ATTENDANCE({EMAIL}, {NAME}, {NUMBER}, {SECTION}, {STARTTIME}, {ENDTIME}, {ROOM}, {DAYS}); COMMIT;
...
AS $CREATE_CLASS$
  ...
  BEGIN
    INSERT INTO ROOM_SCHEDULE (ROOM_ID, STARTTIME, ENDTIME, CREATEDON, CREATEDBY, LASTUPDATEON, LASTUPDATEDBY, ACTIVE)
      VALUES (P_ROOM_ID, CAST(P_CLASS_STARTTIME AS TIMESTAMPTZ(0)), CAST(P_CLASS_ENDTIME AS TIMESTAMPTZ(0)), NOW()::TIMESTAMPTZ(0), 'CREATE_CLASS', NOW()::TIMESTAMPTZ(0), 'CREATE_CLASS', TRUE);
    ...
    FROM
      PUBLIC.PROFESSOR P
    WHERE
      P.EMAILADDRESS = P_PROFESSOR_EMAIL;

    IF (V_ROOM_SCHEDULE_ID IS NOT NULL  AND V_PROFESSOR_ID IS NOT NULL) THEN
